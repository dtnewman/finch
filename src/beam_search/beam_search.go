...
func main() {
	...
	best_solution, highest_score := beam_search(2,simple_evaluation,simple_create_random_start,simple_get_neighbors)
	fmt.Println("beam search results", best_solution, highest_score)

	fmt.Println("\nRUN ON TSP")
	tsp_setup_data()
	best_solution, highest_score = beam_search(100,tsp_evaluation,tsp_create_random_start,tsp_get_neighbors)
	fmt.Println("beam search results", best_solution, -highest_score)
